...
function createProject(templet, devDir, name) {
    ...
    packages._core.src([`${templet}/scss/_base.scss`, `${templet}/scss/_reset.scss`])
    ...
    packages._core.src(`${templet}/js/main.tmpl`)
        ...
        .pipe(packages._replace('@@project', letter_name.replace(/(\w)/, v=> v.toUpperCase())))
        .pipe(packages._replace('@@letter', letter_name))
